...
bool TimedAndPhysicalDualTrack::SyncAndDemultiPhysicalToTimed(const int trackLen)
{
    ...
    lastTimedAndPhysicalTrackSingleLocal.syncAndDemultiThisTrackToOffset(0, trackLen); // Only demulting here.
    ...
    if (!lastTimedAndPhysicalTrackSingleLocal.track.findSyncOffsetComparedTo(timedTrack, syncOffset)) // Can not sync.
        return false;
    lastTimedAndPhysicalTrackSingleLocal.syncAndDemultiThisTrackToOffset(syncOffset, trackLen); // Syncing here.
