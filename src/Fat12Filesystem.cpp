...
Fat12Filesystem::Fat12Filesystem(const Format& fmt, Disk& disk)
    : fmt(fmt), disk(disk), new_boot_sector_data(fmt.sector_size(), fmt.fill),
    bpb(*reinterpret_cast<BIOS_PARAMETER_BLOCK*>(new_boot_sector_data.data()))
{
}
...
bool Fat12Filesystem::GetLogicalSector(int sector_index, const Sector*& found_sector)
{
    if (sector_index < 0 || sector_index > fmt.disk_size() / fmt.sector_size())
        throw util::exception("invalid logical sector index %s", sector_index);
    auto sector_id = sector_index % fmt.sectors + 1;
    sector_index /= fmt.sectors;
    auto head = sector_index % fmt.heads;
    auto cyl = sector_index / fmt.heads;
    // Finding sector this way because it might be missing.
    return disk.find(Header(cyl, head, sector_id, fmt.size), found_sector);
}

bool Fat12Filesystem::PrepareBootSector()
{
    ...
    if (GetLogicalSector(0, boot_sector))
    {
        ...
        const auto& data = boot_sector->data_copy();
        std::copy(data.begin(), data.begin() + std::min(data.size(), new_boot_sector_data.size()), new_boot_sector_data.begin());
        return true;
    }
    else if (fmt.base > 1 || fmt.sectors <= 0) // Boot sector is not requested and is missing, it is a failure.
    {
        throw util::exception("missing boot sector is unsuitable for st output, specifying base=1 might help");
    }
    return false;
}
...
int Fat12Filesystem::AnalyseFatSectors()
{
    ...
    std::vector<double> fat_sector_match(max_fat_sectors + 1);
    std::vector<int> fat_sector_participants(max_fat_sectors + 1);
    std::vector<const Sector*> logical_sectors(fat1_sector_0_index + max_fat_sectors * 2);
    ...
    for (int fat_sector_i = fat1_sector_0_index; fat_sector_i < fat1_sector_0_index + max_fat_sectors * 2; fat_sector_i++)
        logical_sectors[fat_sector_i] = GetLogicalSector(fat_sector_i, fat_sector) ? fat_sector : nullptr;
...
void Fat12Filesystem::ReconstructBpb()
{
    ...
    if (fat_sectors < 1 || fat_sectors > 10 || sectors_per_cluster < 1 || sectors_per_cluster > 4) {
        ...
        util::store_le_value(new_root_dir_entries, bpb.abRootDirEnts); // It is 0x70 or 0xe0 normally.
                                                                        // It is usually 2 but sometimes 1 when one wants less loss per cluster but it requires bigger FAT.
        bpb.bSecPerClust = DetermineSectorsPerCluster();
    }
}
